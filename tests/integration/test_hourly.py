...
def test_hourly(mocker):
    ...
    mock_fetch.return_value = pd.read_pickle(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'fixtures', 'df_hourly.pickle'))
...
def test_hourly_timezone(mocker):
    ...
    mock_fetch.return_value = pd.read_pickle(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'fixtures', 'df_hourly.pickle'))
    ts = ms.hourly("10637", datetime(2020, 1, 1, 15), datetime(2020, 1, 1, 17), timezone='Europe/Berlin')
    df = ts.fetch()
...
def test_hourly_multiple(mocker):
    ...
    mock_metar_fetch.return_value = pd.read_pickle(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'fixtures', 'df_metar.pickle'))
    mock_model_fetch.return_value = pd.read_pickle(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'fixtures', 'df_model.pickle'))
