...
class CacheService:
    ...
    @classmethod
    def persist(cls, path: str, data: pd.DataFrame | dict | list, type: str) -> None:
        """
    ...
    @classmethod
    def from_func(cls, func, args, kwargs, ttl: int, format: str) -> pd.DataFrame | dict | list:
        """
        Cache a function's return value
        """
        uid = cls.func_to_uid(func, args, kwargs) # Get UID for function call
        path = cls.get_cache_path(uid, format) # Get the local cache path
        result = cls.fetch(path, format) if ttl > 0 and exists(path) and not cls.is_stale(path, ttl) else False
        ...
        if isinstance(result, pd.DataFrame) or result:
            ...
            if ttl > 0:
                cls.persist(path, result, format)
